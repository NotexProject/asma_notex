...
public class Main implements Initializable {
    ...
    private ObservableList<Button> Commentaire = FXCollections.observableArrayList();
    private ObservableList<Button> evenementsItems = FXCollections.observableArrayList();
    ...
    private HBox main;
    @FXML
    private TitledPane controls1;
    @FXML
    private TitledPane controls11;
    @FXML
    private TitledPane charts1;
    @FXML
...
//        Popover.ctrl.options.getChildren().clear();
    ...
    private void barInitial() {
        ...
        ((VBox) controls1.getContent()).getChildren().setAll(Commentaire);
        ((VBox) controls11.getContent()).getChildren().setAll(evenementsItems);
    ...
    private void populateItems() {
        ...
        for (Node node : ((VBox) controls1.getContent()).getChildren()) {
            Commentaire.add((Button) node);
            items.add((Button) node);
        }
        for (Node node : ((VBox) controls11.getContent()).getChildren()) {
            evenementsItems.add((Button) node);
            items.add((Button) node);
        }
    ...
    private void commentaire() {
        body.setContent(ViewManager.getInstance().get("commentaire"));
        title.setText("Commentaire");
    }
    
        @FXML
    private void evenements() {
        body.setContent(ViewManager.getInstance().get("evenements"));
        title.setText("evenements");
    }
         @FXML
    private void participation() {
        body.setContent(ViewManager.getInstance().get("participation"));
        title.setText("participation");
    }
             @FXML
    private void participer() {
        body.setContent(ViewManager.getInstance().get("participer"));
        title.setText("participer");
    }
